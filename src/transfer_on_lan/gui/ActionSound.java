...
public class ActionSound implements ActionListener {
    ...
    public ActionSound(PanPreferences pp) {
        this.pp = pp;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        choice.setFileSelectionMode(JFileChooser.FILES_ONLY); // permet de choisir uniquement des fichiers
        choice.setMultiSelectionEnabled(false);

        FileFilter filtre = new FileNameExtensionFilter("Fichiers aux formats wav", "wav");
        choice.setFileFilter(filtre);

        int response = choice.showOpenDialog(this.pp.getMain().getWindow());
        if (response == JFileChooser.APPROVE_OPTION) {
            this.pp.setTxtSound(choice.getSelectedFile().getAbsolutePath());
        }
    }
}
